...
async def audio_source_from_query(query, server):
    with session_scope() as session:
        ...
        try:
            db_query = (
                session.query(Bind).filter_by(alias=query).filter_by(server=server)
            )
            bind = db_query.one()
            session.close()
        except Exception:
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def from_url_with_timestamp(
        cls, url, *, loop=None, stream=False, start=0, stop=10000
    ):
        loop = loop or asyncio.get_event_loop()
        data = await loop.run_in_executor(
            None, lambda: ytdl.extract_info(url, download=not stream)
    ...
    async def yt_video_download(cls, url, loop=None):
        loop = loop or asyncio.get_event_loop()
        data = await loop.run_in_executor(
            None, lambda: ytdl_video.extract_info(url, download=True)
        )
    ...
    async def video_details(cls, url, loop=None):
        loop = loop or asyncio.get_event_loop()
        data = await loop.run_in_executor(
            None, lambda: ytdl_video.extract_info(url, download=False)
        )
        if "entries" in data:
            # take first item from a playlist
            data = data["entries"][0]
